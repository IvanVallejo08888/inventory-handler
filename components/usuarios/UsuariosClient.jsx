...
const VACIO_EDITAR = { id:null, nombreCompleto:'', identificacion:'', celular:'', tipoSangre:'', correo:'', rol:'VENDEDOR', activo:'true', nuevaContrasena:'', confirmarContrasena:'' };
...
export default function UsuariosClient({ lista, totalUsuarios, totalAdmins, totalVendedores, buscar, filtroRol, sesionId }) {
  ...
  function abrirEditar(u) {
    setForm({ id: u.id, nombreCompleto: u.nombreCompleto, identificacion: u.identificacion,
  ...
  async function enviar(accion) {
    ...
    try {
      ...
      if (!res.ok || data.error) { setMsgTipo('error'); setMsg(data.error || 'Error.'); }
        ...
        setModalEditar(null); setConfirmId(null);
    ...
    <div className="content-area">
      ...
      <form method="GET" className="search-bar">
        ...
        {(buscar || filtroRol !== 'TODOS') && (
          <a href="/usuarios" className="btn btn-secondary" style={{ padding:'0.5rem 1rem', fontSize:'0.85rem' }}>Limpiar</a>
        )}
      </form>
      ...
      <div className="panel">
        ...
        <div className="table-responsive">
          <table className="area17-table">
            <thead>
              <tr><th>Usuario</th><th>Identificación</th><th>Correo</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {lista.map(u => (
                <tr key={u.id}>
                  <td>
                    <div style={{ display:'flex', alignItems:'center', gap:'0.6rem' }}>
                      ...
                      {u.id === SUPERADMIN_ID && <span style={{ fontSize:'0.65rem', color:'var(--primary)', fontWeight:700 }}>SUPERADMIN</span>}
                    </div>
                  </td>
                  <td style={{ fontFamily:'monospace', fontSize:'0.85rem' }}>{u.identificacion}</td>
                  <td style={{ fontSize:'0.82rem' }}>{u.correo}</td>
                  <td><span className={u.rol === 'ADMINISTRADOR' ? 'badge-activo' : ''}
                      style={u.rol !== 'ADMINISTRADOR' ? { color:'var(--text-secondary)', fontSize:'0.82rem' } : {}}>
                      {u.rol}
                  </span></td>
                  <td><span className={u.activo ? 'badge-activo' : 'badge-inactivo'}>{u.activo ? 'Activo' : 'Inactivo'}</span></td>
                  <td>
              ...
              {!lista.length && <tr><td colSpan={6} style={{ textAlign:'center', color:'var(--text-muted)', padding:'2rem' }}>Sin usuarios</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {modalEditar && (
        <div className="confirm-overlay active" onClick={() => setModalEditar(null)}>
          <div className="modal-box" style={{ maxWidth:540 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ padding:'1.25rem 1.5rem', borderBottom:'1px solid var(--border-subtle)', margin:0 }}>Editar usuario</h3>
            <div style={{ padding:'1.5rem' }}>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Nombre completo</label>
                  <input className="form-control" value={form.nombreCompleto} onChange={e => set('nombreCompleto', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">Identificación</label>
                  <input className="form-control" value={form.identificacion} onChange={e => set('identificacion', e.target.value)} />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Celular</label>
                  <input className="form-control" value={form.celular} onChange={e => set('celular', e.target.value)} />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input className="form-control" type="email" value={form.correo} onChange={e => set('correo', e.target.value)} />
              </div>
              <div className="form-row">
                <div className="form-group">
                  ...
                  {form.id === sesionId && <p className="readonly-note">No puedes cambiar tu propio rol</p>}
                </div>
                <div className="form-group">
              ...
              <p style={{ fontSize:'0.78rem', fontWeight:700, color:'var(--primary)', textTransform:'uppercase', borderTop:'1px solid var(--border-color)', paddingTop:'1rem', marginTop:'0.5rem', marginBottom:'0.75rem' }}>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <div style={{ position:'relative' }}>
                    <input className="form-control" type={mostrarPw ? 'text' : 'password'}
                      value={form.nuevaContrasena} onChange={e => set('nuevaContrasena', e.target.value)}
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" type="password"
                    value={form.confirmarContrasena} onChange={e => set('confirmarContrasena', e.target.value)} />
      ...
      {confirmId && (
        <div className="confirm-overlay active">
          <div className="confirm-box">
            ...
            <p>Esta acción no se puede deshacer. El usuario perderá acceso al sistema.</p>
