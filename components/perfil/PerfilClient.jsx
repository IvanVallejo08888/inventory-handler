...
export default function PerfilClient({ sesion }) {
  ...
  const [formPw, setFormPw] = useState({ contrasenaActual:'', nuevaContrasena:'', confirmarContrasena:'' });

  const fotoUrl = sesion.fotoPerfil
    ? `/api/foto-perfil?archivo=${sesion.fotoPerfil}&t=${Date.now()}`
    : null;
  ...
  async function guardarDatos() {
    setCargando(true);
    try {
      const res  = await fetch('/api/perfil', { method:'POST', headers:{'Content-Type':'application/json'},
  ...
  async function cambiarContrasena() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || data.error) { setMsgTipo('error'); setMsg(data.error || 'Error.'); }
      else { setMsgTipo('success'); setMsg('Contraseña actualizada exitosamente.'); setFormPw({ contrasenaActual:'', nuevaContrasena:'', confirmarContrasena:'' }); }
    } catch { setMsgTipo('error'); setMsg('Error de conexión.'); }
    finally   { setCargando(false); }
  }
    ...
    <div className="content-area">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', gap:'1.5rem', alignItems:'start' }}>
        ...
        <div className="panel" style={{ textAlign:'center', padding:'2rem 1.5rem' }}>
          {avatarUrl
            ? <img src={avatarUrl} alt="Foto" className="foto-avatar" style={{ width:120, height:120, borderRadius:'50%', objectFit:'cover', margin:'0 auto 1rem', display:'block', border:'3px solid var(--primary)', boxShadow:'0 0 20px var(--primary-glow)' }} />
            : <div className="foto-iniciales" style={{ width:120, height:120, borderRadius:'50%', margin:'0 auto 1rem', display:'flex', alignItems:'center', justifyContent:'center', fontSize:40, fontWeight:900 }}>
                {getIni(sesion.nombreCompleto)}
              </div>
          }
          <div style={{ fontWeight:700, marginBottom:'0.25rem', color:'var(--text-primary)' }}>{sesion.nombreCompleto}</div>
          <div style={{ fontSize:'0.78rem', color:'var(--primary)', fontWeight:600, marginBottom:'1.5rem' }}>{sesion.rol}</div>

          <label style={{ display:'block', cursor:'pointer', background:'var(--bg-input)', border:'1px dashed var(--border-glow)', borderRadius:'0.6rem', padding:'0.75rem', marginBottom:'0.75rem' }}>
            <input type="file" accept="image/jpeg,image/png,image/webp" style={{ display:'none' }} onChange={onFotoChange} />
            <span style={{ fontSize:'0.82rem', color:'var(--text-secondary)' }}>
              {fotoFile ? `📎 ${fotoFile.name}` : '📷 Seleccionar foto (JPG, PNG, WEBP — máx 2 MB)'}
            </span>
          </label>
              ...
              <div className="form-group">
                ...
                <div style={{ position:'relative' }}>
                  <input className="form-control" type={mostrarPw ? 'text':'password'}
                    value={formPw.contrasenaActual} onChange={e => setPw('contrasenaActual', e.target.value)}
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="form-control" type="password" placeholder="Mínimo 8 caracteres"
                    value={formPw.nuevaContrasena} onChange={e => setPw('nuevaContrasena', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">Confirmar contraseña *</label>
                  <input className="form-control" type="password"
                    value={formPw.confirmarContrasena} onChange={e => setPw('confirmarContrasena', e.target.value)} />
