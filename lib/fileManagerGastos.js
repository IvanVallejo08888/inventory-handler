...
export async function totalGastosDia() {
  const dia = hoy();
  const rows = await sql`SELECT COALESCE(SUM(valor), 0)::float AS t FROM gastos WHERE estado = 'ACTIVO' AND fecha = ${dia}`;
  return rows[0].t;
}

// Desglose de los gastos del día por medio de pago: EFECTIVO, NEQUI, BANCOLOMBIA, DAVIPLATA.
// Los gastos MIXTOS se reparten entre EFECTIVO y el medio de pago de la transferencia.
export async function gastosPorMedioPagoDia() {
  await ensureSchemaMetodoPago();
  const dia = hoy();
  const rows = await sql`
    SELECT metodo_pago, medio_pago, valor, valor_efectivo, valor_transferencia
    FROM gastos WHERE estado = 'ACTIVO' AND fecha = ${dia}
  `;
  const mapa = {};
  const add = (key, val) => {
    if (!val) return;
  ...
  for (const r of rows) {
    if (r.metodo_pago === 'TRANSFERENCIA') {
      add(r.medio_pago || 'OTRO', parseFloat(r.valor) || 0);
    } else if (r.metodo_pago === 'MIXTO') {
      add('EFECTIVO', parseFloat(r.valor_efectivo) || 0);
      add(r.medio_pago || 'OTRO', parseFloat(r.valor_transferencia) || 0);
    } else {
      add('EFECTIVO', parseFloat(r.valor) || 0);
    }
  }
  return mapa;
}
