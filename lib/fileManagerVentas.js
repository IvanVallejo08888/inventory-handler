...
import { descontarStock, agregarStock, descontarStockVariante, agregarStockVariante, buscarPorCodigo } from './fileManagerProductos.js';
import { fechaHoyColombia, horaActualColombia, fechaHaceDiasColombia, semanaActualColombia } from './fechaColombia.js';
...
// Auto-migración idempotente: agrega las columnas de proveedor de transferencia si la BD aún no las tiene.
// Nullable para no afectar ventas históricas.
let schemaTransferProviderReady = null;
function ensureSchemaTransferProvider() {
  if (!schemaTransferProviderReady) {
    schemaTransferProviderReady = sql`
      ALTER TABLE ventas
        ADD COLUMN IF NOT EXISTS transfer_provider TEXT,
        ADD COLUMN IF NOT EXISTS mixed_transfer_provider TEXT
    `.catch(err => { schemaTransferProviderReady = null; throw err; });
  }
  return schemaTransferProviderReady;
}

// Auto-migración idempotente: agrega la columna `talla` a detalles_ventas (para devolver
// stock a la variante correcta al cancelar una venta).
let schemaTallaDetalleReady = null;
function ensureSchemaTallaDetalle() {
  if (!schemaTallaDetalleReady) {
    schemaTallaDetalleReady = sql`ALTER TABLE detalles_ventas ADD COLUMN IF NOT EXISTS talla TEXT`
      .catch(err => { schemaTallaDetalleReady = null; throw err; });
  }
  return schemaTallaDetalleReady;
}
...
function mapVenta(row) {
  return {
    ...
    transferProvider:       row.transfer_provider || null,
    mixedTransferProvider:  row.mixed_transfer_provider || null,
  };
}
...
export async function crearVenta(venta, detalles) {
  ...
  await ensureSchemaTransferProvider();
  await ensureSchemaTallaDetalle();
  ...
  const ventaCompleta = {
    ...
    transferProvider:       venta.transferProvider      || null,
    mixedTransferProvider:  venta.mixedTransferProvider || null,
  };
  ...
  await sql`
    INSERT INTO ventas (id, codigo, fecha, hora, vendedor_id, vendedor_nombre, subtotal,
                        ...
                        costo_adicional, costo_adicional_tipo, transfer_provider, mixed_transfer_provider)
    VALUES (${ventaCompleta.id}, ${ventaCompleta.codigo}, ${ventaCompleta.fecha}, ${ventaCompleta.hora},
            ...
            ${ventaCompleta.transferProvider}, ${ventaCompleta.mixedTransferProvider})
  `;
  ...
  for (const d of detalles) {
    ...
    d.ventaId = id;
    await sql`
      INSERT INTO detalles_ventas (id, venta_id, producto_codigo, producto_nombre, cantidad, precio_unitario, descuento_unidad, subtotal, talla)
      VALUES (${d.id}, ${d.ventaId}, ${d.productoCodigo}, ${d.productoNombre}, ${d.cantidad},
              ${d.precioUnitario}, ${d.descuentoUnidad || 0}, ${d.subtotal}, ${d.talla || null})
    `;
    if (d.talla) {
      const producto = await buscarPorCodigo(d.productoCodigo);
      if (producto) await descontarStockVariante(producto.id, d.talla, d.cantidad);
    } else {
...
export async function cancelarVenta(id, vendedorId = null) {
  ...
  for (const d of detalles) {
    if (d.talla) {
      const producto = await buscarPorCodigo(d.productoCodigo);
      if (producto) await agregarStockVariante(producto.id, d.talla, d.cantidad);
    } else {
      await agregarStock(d.productoCodigo, d.cantidad);
    }
  }
...
export async function ventasSemana() {
  const { lunes, domingo } = semanaActualColombia();
  const rows = await sql`SELECT * FROM ventas WHERE fecha >= ${lunes} AND fecha <= ${domingo} AND estado = 'COMPLETADA' ORDER BY id DESC`;
  return rows.map(mapVenta);
}
...
export async function ventasPorDia() {
  ...
  const rows = await sql`
    ...
    FROM ventas
    WHERE fecha >= ${lunes} AND fecha <= ${domingo} AND estado = 'COMPLETADA'
    GROUP BY fecha
    ORDER BY fecha
  `;
...
// Desglose de las transferencias del día por entidad: NEQUI, BANCOLOMBIA, DAVIPLATA.
export async function transferenciaPorEntidadHoy() {
  await ensureSchemaTransferProvider();
  const h = hoyStr();
  const rows = await sql`
    SELECT tipo_pago, transfer_provider, mixed_transfer_provider, valor_transferencia
    FROM ventas WHERE fecha = ${h} AND estado = 'COMPLETADA'
  `;
  const mapa = {};
  for (const r of rows) {
    const val = parseFloat(r.valor_transferencia) || 0;
    if (!val) continue;
    const provider = r.tipo_pago === 'MIXTO' ? r.mixed_transfer_provider : r.transfer_provider;
    const key = provider || 'OTRO';
