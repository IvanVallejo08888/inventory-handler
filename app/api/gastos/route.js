...
export async function POST(request) {
  ...
  try {
    ...
    if (accion === 'agregar') {
      ...
      const res = crear({ nombre, valor, fecha: body.fecha, categoria, descripcion });
    ...
    if (accion === 'editar') {
      ...
      const res = actualizar(id, { nombre, valor, fecha: body.fecha, categoria, descripcion });
      if (res.error) return NextResponse.json(res, { status: 400 });
      return NextResponse.json(res);
    }
